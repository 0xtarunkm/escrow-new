...
import { EscrowNew } from '../anchor/idlType';
import idl from '@/utils/anchor/idl.json';
...
export const useEscrow = () => {
  ...
  const getMintInfo = async (mint: PublicKey) => {
    ...
    return getMint(anchorProvider.connection, mint, 'confirmed', tokenProgram);
  };
  ...
  const getAllEscrowAccounts = async () => {
    ...
    console.log(escrowAccounts[1].publicKey.toString());
